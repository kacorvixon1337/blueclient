...
public class ESPUtil{
    ...
    public static Vector3f projectOn2D(float x, float y, float z, int scaleFactor) {
        ...
        if (GLU.gluProject(x, y, z, floatBuffer1, floatBuffer2, intBuffer, windPos)) {
            return new Vector3f(windPos.get(0) / scaleFactor, (Minecraft.getMinecraft().displayHeight - windPos.get(1)) / scaleFactor, windPos.get(2));
        }
        return null;
    }

    public static double[] getInterpolatedPos(Entity entity) {
        float ticks = Minecraft.getMinecraft().timer.renderPartialTicks;
        return new double[]{
                MathUtil.interpolate(entity.lastTickPosX, entity.posX, ticks) - Minecraft.getMinecraft().getRenderManager().viewerPosX,
                MathUtil.interpolate(entity.lastTickPosY, entity.posY, ticks) - Minecraft.getMinecraft().getRenderManager().viewerPosY,
                MathUtil.interpolate(entity.lastTickPosZ, entity.posZ, ticks) - Minecraft.getMinecraft().getRenderManager().viewerPosZ
        };
    }
