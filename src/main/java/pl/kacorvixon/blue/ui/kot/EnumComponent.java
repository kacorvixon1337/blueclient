...
public final class EnumComponent extends pl.kacorvixon.blue.ui.kot.Component implements PropertyComponent {
    ...
    public EnumComponent(Component parent, EnumProperty<?> property, int x, int y, int width, int height) {
    ...
    public void drawComponent(ScaledResolution scaledResolution, int mouseX, int mouseY) {
       ...
       buttonBottom = (getY() + getHeight() / 2 + (getHeight() / 2 / 2) + 2);
        Hud.getFontRenderer().drawStringWithShadow(property.value.name(), x + width - Hud.getFontRenderer().getWidth(property.value.name()) + 10, buttonBottom = (int) (y + getHeight() / 2 - 3), -1);
    }

    @Override
    public void onMouseClick(int mouseX, int mouseY, int button) {
        if (button == 0 && mouseX > buttonLeft - Hud.getFontRenderer().getWidth(property.value.name()) && mouseY > buttonTop - 8 && mouseX < buttonRight && mouseY < buttonBottom + 8)
            property.increment();
    }
